...
public class itHero extends itAgent {
   ...
   // static final String LOOKS = LOOKS;
    ...
    void advance(String powers) {
        ...
        if (powers != null) {
            for (int i = 0; i < Constants.TraitList.length; i++) {
                if (powers.indexOf(Constants.TraitList[i]) >= 0 || powers.indexOf(Constants.TraitStub[i]) >= 0) {
    ...
    public String rankString() {
        getPack();
        calcCombat();
        return String.valueOf(String.valueOf(new StringBuffer("{itList|").append(getName()).append("|").append(getSocial()).append("|").append(getAge()).append("|").append(getFame()).append("|").append(getSkill()).append("|").append(getAttack() + getDefend()).append("|").append(getGuts() + getWits() + getCharm()).append("|").append(getMoney() + getStore().getCount("Marks")).append("|").append(guildRank()).append("|").append(itAgent.DEAD.equals(getState()) ? getState() : getPlace()).append("|").append(getClan() == null ? Constants.NONE : getClan()).append("}")));
    }
    ...
    public String getFullTitle() {
        return String.valueOf(String.valueOf(new StringBuffer(String.valueOf(String.valueOf(getRankTitle()))).append(" ").append(getName())));
    }
    ...
    public void resetActions() {
        if (hasTrait("Blind")) {
            ...
            getTemp().fix(Constants.ACTIONS, 1 + (fightRank() / 4) + (thiefRank() / 5) + (magicRank() / 6));
        }
        getActions().clrQueue();
    ...
    public int packMagic() {
        return packCount(GearTypes.BLIND_DUST) + packCount(GearTypes.PANIC_DUST) + packCount(GearTypes.BLAST_DUST);
    }
    ...
    public Screen killedScreen(Screen from, String tale, boolean losePack) {
        ...
        if (getPack().getCount("Bottled Faery") > 0) {
            msg = msg2 == null ? "\tYou are wounded mortally and fall to the ground.  Before the creature can act, a Bottled Faery breaks free from your pack and transports you the monastery.  It clucks at you briefly in admonishment, then flies away leaving a trail of sparkly dust.\n" : String.valueOf(String.valueOf(msg2)).concat("\n\tA Bottled Faery breaks free from your pack and transports you to the healers!");
            subPack("Bottled Faery", 1);
        } else {
            if (msg2 == null) {
                msg2 = "\tYou are wounded mortally and fall to the ground.  The creature roots through your pack and leaves you for dead.  A friendly woodsman finds you before the last of your blood has fallen.  He drags you to the healers where your life is sustained by a thread.\n";
            }
            ...
            msg = String.valueOf(String.valueOf(msg2)).concat(String.valueOf(String.valueOf(String.valueOf(String.valueOf(new StringBuffer("\n\t\t*** You lose ").append(cost).append(" quests. ***\n"))))));
        }
