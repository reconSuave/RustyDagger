...
public class itMonster extends itAgent {
  ...
  public String toString(int depth) {
    return String.valueOf(
            String.valueOf(
                String.valueOf(
                        String.valueOf(
                            String.valueOf(String.valueOf(toStringHead(depth)))
                                .concat(
                                    String.valueOf(
                                        String.valueOf(
                                            String.valueOf(
  ...
  public void fixLists() {
    ...
    if (this.text == null) {
      System.err.println(
          String.valueOf(
              String.valueOf(new StringBuffer("ERR: [text=null] for [").append(this).append("]"))));
    }
    Item it = find("pic");
    if (it == null) {
      System.err.println(
          String.valueOf(
              String.valueOf(new StringBuffer("ERR: [pic=null] for [").append(this).append("]"))));
    } else {
    ...
    if (this.text == null) {
      System.err.println(
          String.valueOf(
              String.valueOf(new StringBuffer("ERR: [opts=null] for [").append(this).append("]"))));
    }
  }
  ...
  void calcPrimary(int weight) {
    ...
    if (hasTrait("adjust")) {
      float ratio2 =
          (1.0f + (((float) weight) * 0.1f)) / ((float) (getPower() / Tools.getHero().getPower()));
      if (ratio2 > 1.0f) {
        alterGuts(ratio2);
        alterWits(ratio2);
        alterCharm(ratio2);
  ...
  void calcSecondary(int weight) {
    ...
    fixStatus(
        Constants.FAME,
        (((getGuts() + getWits()) + getCharm()) / 30)
            + ((((thief() + magic()) + fight()) + weight) / 4));
    ...
    if ("aggressive".equals(passion)) {
      ...
      System.err.println(
          String.valueOf(
              String.valueOf(
                  new StringBuffer("Unknown [passion=")
                      .append(passion)
                      .append("] for [")
                      .append(this)
                      .append("]"))));
    ...
    for (int ix = 0; ix < from.getCount(); ix++) {
      ...
      if (GearTable.find(it)) {
        if (it instanceof itNote) {
          ...
          } else if (Tools.percent(it.getCount())
  ...
  void testItem(Item it, String type) {
    if (it == null) {
      System.err.println(
          String.valueOf(
              String.valueOf(
                  new StringBuffer("ERR: No ").append(type).append(" for ").append(getName()))));
    } else if (!(it instanceof itCount)) {
      System.err.println(
          String.valueOf(
              String.valueOf(
                  new StringBuffer("ERR: Bad ").append(type).append(" for ").append(getName()))));
    ...
    return String.valueOf(
        String.valueOf(
            new StringBuffer("\tThe ")
                .append(getName())
                .append(" steals and devours ")
                .append(num)
                .append(" pieces of rope!  Baa-a-a-a!\n")));
  }
    ...
    return String.valueOf(
        String.valueOf(
            new StringBuffer("\tThe ")
                ...
                .append(" rips the ")
                .append(it.getName())
                .append(" from your body and swallows it whole!\n")));
  }
}
