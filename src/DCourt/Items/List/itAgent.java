...
public abstract class itAgent extends itList implements Constants, GearTypes, ArmsTrait {
  ...
  public itAgent(itAgent it) {
    super(it);
    setVals(
        it.getGuts(), it.getWits(), it.getCharm(), it.getAttack(), it.getDefend(), it.getSkill());
    fixLists();
  }
  ...
  public int packHeal() {
    return this.pack.getCount(GearTypes.APPLE)
        + this.pack.getCount(GearTypes.TROLL)
        + this.pack.getCount(GearTypes.SALVE);
  }

  public int packMagic() {
    return this.pack.getCount(GearTypes.BLIND_DUST)
        + this.pack.getCount(GearTypes.PANIC_DUST)
        + this.pack.getCount(GearTypes.BLAST_DUST);
  }
