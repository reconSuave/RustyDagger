...
public abstract class Shop extends Indoors {
    ...
    public void createTools() {
        ...
        this.special = null;
        if (getSpecial() != null) {
            this.special = new Button(String.valueOf(String.valueOf(getSpecial())).concat(String.valueOf(String.valueOf(0))));
    ...
    public void updateTools() {
        if (this.special != null) {
            ...
            this.special.setLabel(String.valueOf(String.valueOf(new StringBuffer(String.valueOf(String.valueOf(getSpecial()))).append(" $").append(cost))));
    ...
    public String shopName(Item it) {
        String msg = it instanceof itArms ? it.toShow() : String.valueOf(String.valueOf(new StringBuffer(String.valueOf(String.valueOf(it.getName()))).append("(").append(Screen.packCount(it)).append(")")));
        return isPack() ? String.valueOf(String.valueOf(msg)).concat(String.valueOf(String.valueOf(" $".concat(String.valueOf(String.valueOf(packValue(it))))))) : String.valueOf(String.valueOf(msg)).concat(String.valueOf(String.valueOf(" $".concat(String.valueOf(String.valueOf(stockValue(it)))))));
    }

    public int packValue(Item it) {
        int cost = stockValue(it);
        int cost2 = Screen.hasTrait(Constants.MERCHANT) ? (cost * this.RESALE) / 95 : (cost * this.RESALE) / 100;
    ...
    boolean discardItem(Item it) {
        ...
        if (!it.isMatch("Marks") && stockValue(it) >= 1 && !discardPack(it)) {
            return this.sellList.find(it.getName()) == null && this.buyList != null && this.buyList.find(it.getName()) == null;
        }
        return true;
    }
