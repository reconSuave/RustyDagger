...
public class arPostal extends Indoors {
    ...
    FTextList postbox;
    itList mail = null;
    static String[] greeting = {null, "In a minute..", "Okay, alright already.", "Fill in this form.", "This form is wrong", "I'm on my break", "Geez, again?", "*Sigh* Oh I suppose.", "Right now? Yeah, yeah."};
    ...
    void loadMailList() {
        this.mail = (itList) Loader.cgiItem(Loader.LISTMAIL, String.valueOf(String.valueOf(new StringBuffer(String.valueOf(String.valueOf(Screen.getHero().getName()))).append("|").append(Screen.getPlayer().getSessionID()))));
    }
    ...
    Screen takePackage() {
        ...
        Buffer buf = Loader.cgiBuffer(Loader.TAKEMAIL, String.valueOf(String.valueOf(new StringBuffer(String.valueOf(String.valueOf(h.getName()))).append("|").append(Screen.getSessionID()).append("|").append(index))));
        String msg = this.postbox.getItem(index);
        this.postbox.delItem(index);
        if (buf == null || buf.isEmpty() || buf.isError()) {
            return new arNotice(this, String.valueOf(String.valueOf(new StringBuffer("A transmission error has occurred:\n").append(buf == null ? "" : buf.peek()).append("\n").append("Sorry About That."))));
        }
        String msg2 = String.valueOf(String.valueOf(new StringBuffer("\tA mail daemon pushes out a dilapidated package, makes a futile attempt to polish it up, then scurries into the depths of the mail room.\n\nThe label reads:\n\t").append(msg).append("\n").append("The package contains:\n")));
        ...
        for (int ix = 0; ix < list.getCount(); ix++) {
            ...
            if (it.getName() != null && it.getName().length() >= 1) {
                msg2 = String.valueOf(String.valueOf(msg2)).concat(String.valueOf(String.valueOf(String.valueOf(String.valueOf(new StringBuffer("\t").append(it.toShow()).append("\n"))))));
                Screen.addPack(it);
            }
        }
        h.subMoney(100);
