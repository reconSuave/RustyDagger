...
public class arPeer extends arNotice {
    ...
    itHero target;
    int spend;
    static final int SEEKMONEY = 0; // SEEKMONEY;
    static final String SEEKGEM = "SEEKGEM";
    ...
    static final String[] sex_Dress = {"trousers and dark colors", "long skirts and bright colors", "fluffy pants and a puffy shirt", "dark overcoat concealing all"};
    static final String[] situation = {" battling a centaur, bellowing forth ", " fleeing a wyvern, shrieking in terror ", " riding a gryphon above the clouds, exhorting ", " searching the corpse of an elf, mumbling ", " tracking a forest boar, muttering ", " seducing a castle servant, whispering ", " gambling amongst nobles, chuckling ", " training at the guild, while crying ", " sharing a beer in the tavern, while boasting ", " tricking a goblin mage, then saying ", " stroking a goblin queen, then moaning "};
    static final String[] strongAdj = {"stern", "brave", "headstrong", "alert", "valiant", "cunning", "powerful"};
    static final String noDescription = "noDescription";
    // static final String noDescription = noDescription;
    static final String describe0 = "describe0";
    // static final String describe0 = describe0;
    static final String describe1 = "describe1";
    // static final String describe1 = describe1;
    static final String describe2 = "describe2";
    // static final String describe2 = describe2;
    static final String describe3 = "describe3";
    // static final String describe3 = describe3;
    static final String clanmsg = "clanmsg";
    // static final String clanmsg = clanmsg;
    static final String dressmsg = "dressmsg";
    // static final String dressmsg = dressmsg;
    static final String behave1 = "behave1";
    // static final String behave1 = behave1;
    static final String behave2 = "behave2";
    // static final String behave2 = behave2;
    static final String intro1 = "intro1";
    // static final String intro1 = intro1;
    static final String intro2 = "intro2";
    // static final String intro2 = intro2;
    ...
    public void localPaint(Graphics g) {
        ...
        if (this.spend != 4) {
            this.seek.enable(this.heroName.isMatch(h.getName()) || (this.spend == 1 && h.getMoney() > SEEKMONEY) || (this.spend == 2 && h.packCount(SEEKGEM) > 3));
        }
        ...
        if (this.heroName.isMatch(h.getName())) {
            ...
            g.drawString(String.valueOf(String.valueOf(new StringBuffer("Cost: 3/").append(h.packCount(SEEKGEM)).append(" ").append(SEEKGEM))), 195, 23);
    ...
    void LoadVision(String who) {
        ...
        if (who == null || who.length() < 4) {
            setMessage(String.valueOf(String.valueOf(new StringBuffer("Illegal Name: <").append(who).append(">"))));
            ...
            if (buf == null || buf.isEmpty() || buf.isError()) {
                setMessage(String.valueOf(String.valueOf(new StringBuffer("Unable to Load <").append(this.pname).append(">"))));
                return;
            }
    ...
    void BuildDescription() {
        ...
        if (lp == null || lp.getCount() < 1) {
            ...
            if (lp.getValue(Constants.HABIT) == null || !Tools.chance(3)) {
                mad = new MadLib("\t$title$ $name$ $clanmsg$ is a $build$ $race$ with $hair$ hair, $eyes$ eyes, and $skin$ skin. $dress$ $behave$ $intro$ the odd trait of $marks$. Rumor has it that $HE$ is a $sign$. \n\tAs you peer into the gem you espy $him$ $situation$ \"$phrase$\"\n\tGuts: $guts$\tWits: $wits$\tCharm: $charm$\nWeapon: $weapon$\nArmor: $armor$");
            } else {
        ...
        for (int i = 0; i < Constants.TraitList.length; i++) {
            if (this.target.hasTrait(Constants.TraitList[i])) {
                msg = String.valueOf(String.valueOf(msg)).concat(String.valueOf(String.valueOf(String.valueOf(String.valueOf(Constants.TraitList[i])).concat(" "))));
                row++;
                if (row % 6 == 0) {
