...
public abstract class WildsScreen extends Screen {
    ...
    boolean doSearch(int loc) {
        ...
        Tools.setRegion(new arNotice(this, String.valueOf(String.valueOf(markFound(pick))).concat(String.valueOf(String.valueOf(Screen.getHero().gainWits(getPower(loc) + 2))))));
        return true;
    }
    ...
    itMonster selectQuest(int loc, String[] names, int[] weight) {
        ...
        for (int ix = 0; ix < weight.length; ix++) {
            total2 -= weight[ix];
            if (total2 < 0) {
                return Screen.findBeast(String.valueOf(String.valueOf(new StringBuffer(String.valueOf(String.valueOf(getWhere(loc)))).append(":").append(names[ix]))));
            }
        }
        return Screen.findBeast(String.valueOf(String.valueOf(new StringBuffer(String.valueOf(String.valueOf(getWhere(loc)))).append(":").append(names[0]))));
    }
    ...
    protected boolean findLight() {
        return Screen.hasTrait(Constants.CATSEYES) || Screen.findGearTrait(ArmsTrait.GLOWS) != null || Screen.findGearTrait(ArmsTrait.FLAME) != null || Screen.subPack("Torch", 1) > 0;
    }
}
