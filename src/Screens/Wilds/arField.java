...
public class arField extends WildsScreen {
    ...
    Screen enterForest() {
        ...
        if (!Tools.contest(h.getWits(), FINDFOREST)) {
            return h.getLevel() >= 6 ? pickQuest(0) : new arNotice(pickQuest(0), "\tYou start hiking towards the distant woods. You are making good time, when suddenly...");
        }
    ...
    Screen enterMound() {
        return Screen.getHero().getQuests() < 1 ? new arNotice(this, WildsScreen.TOO_TIRED) : new arQuest(this, new arMound(), 2, "Goblin Mound Quest", Screen.findBeast("Mound:Gate"));
    }
    ...
    public Screen pickQuest(int loc) {
        return new arQuest(this, 1, "Fields Quest", selectQuest(beasts, Screen.getHero().getLevel() < 3 ? loweight : hiweight));
    }
}
