...
public class arCastle extends WildsScreen {
    ...
    public void goQuesting(int loc) {
        if (loc < 2) {
            ...
            if (!Tools.contest(Screen.getWits(), FIND_OCEAN)) {
                Tools.setRegion(new arNotice(this, DOCKS_FAILURE.concat(String.valueOf(String.valueOf(Tools.select(oceans))))));
    ...
    public Screen pickQuest(int loc) {
        ...
        return new arQuest(this, next, getPower(loc), String.valueOf(String.valueOf(getWhere(loc))).concat(" Quest"), beast);
    }
}
