...
public class Options extends Canvas {
    ...
    static final String[] OPT_ARRAY = {"bribe", "feed", "riddle", "trade", "help", "seduce", "control", "backstab", "berzerk", "swindle", "ieatsu", "attack", "flee", "fish", "bushido", "capture"};
    ...
    Options() {
        this.OPT_STRING = new String[][]{new String[]{"Bribe with Marks", "Pay for Passage", "Give it Money"}, new String[]{"Feed This Creature", "Tempt With Food", "Throw Some Grub"}, new String[]{"Try to Answer", "Hazard a Guess", "Riddle Me This"}, new String[]{"Bargain", "Barter", "Swap Goods", "Trade"}, new String[]{"Aid the Poor Booger", "Help Out", "Lend a Hand"}, new String[]{"Seduce the Beast", "Use Sex Appeal", "Flirt Lewdly"}, new String[]{"  M:Hypnotize"}, new String[]{"  T:Backstab"}, new String[]{"  F:Berzerk"}, new String[]{"  T:Swindle"}, new String[]{"  S:Ieatsu"}, new String[]{"Slay This Brute", "Attack Yon Beastie", "Assault The Monster", "Kill The Critter", "Smash The Devil"}, new String[]{"Flee For Safety", "Run For Your Life", "Evade With Haste", "Run Away! Run Away!"}, new String[]{"Feed This Creature", "Tempt With Food", "Throw Some Grub"}, new String[]{"Present Your Token", "Display A Token", "Hand Over Token"}, new String[]{"Grab The Sucker", "Bottle This Thing", "Take it Captive"}};
        this.select = -1;
    }
    ...
    public void redraw() {
        ...
        for (int px = 0; px < this.list.getCount(); px++) {
            ...
            switch (ix) {
                case 6:
                    it.setValue(String.valueOf(String.valueOf(new StringBuffer(String.valueOf(String.valueOf(this.OPT_STRING[ix][0]))).append("[").append(h.magic()).append("]"))));
                    break;
                case 7:
                case 9:
                    it.setValue(String.valueOf(String.valueOf(new StringBuffer(String.valueOf(String.valueOf(this.OPT_STRING[ix][0]))).append("[").append(h.thief()).append("]"))));
                    break;
                case 8:
                    it.setValue(String.valueOf(String.valueOf(new StringBuffer(String.valueOf(String.valueOf(this.OPT_STRING[ix][0]))).append("[").append(h.fight()).append("]"))));
                    break;
                case 10:
                    it.setValue(String.valueOf(String.valueOf(new StringBuffer(String.valueOf(String.valueOf(this.OPT_STRING[ix][0]))).append("[").append(h.ieatsu()).append("]"))));
                    break;
            }
        }
        setdraw();
    ...
    public void fixList() {
        ...
        if (h.hasTrait("Panic")) {
            ...
            for (int ix = 0; ix < this.goal.getCount(); ix++) {
                ...
                if (choice >= 0 && ((choice != 0 || h.getMoney() >= 1) && ((choice != 3 || h.getMoney() >= 1) && ((choice != 1 || h.packCount(GearTypes.FOOD) >= 1) && ((choice != 13 || h.packCount(GearTypes.FISH) >= 1) && (choice != 14 || (h.packCount(GearTypes.TOKEN) >= 1 && h.getQuests() >= 10 && h.guildRank() < h.getLevel()))))))) {
